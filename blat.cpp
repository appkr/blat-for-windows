...
void printMsg(char *p, ... );							 // Added 23 Aug 2000 Craig Morrison
			 ...
			 char **envp )									 /* Array of environment variable strings    */
{
	...
	if ( (!mime) && (!base64) ) {
		if ( attach>0 ) {
			...
			sprintf( tmpstr, "Content-Type: text/%s; charset=EUC-KR\r\n", textmode);	  // modified 15. June 1999 by JAG
			strcat( header2, tmpstr );
	...
	if ( mime ) {
		...
		sprintf( tmpstr, "MIME-Version: 1.0\r\n" );
		strcat( header, tmpstr );
		sprintf( tmpstr1, "Content-Type: text/%s; charset=EUC-KR\r\n", textmode);	  // modified 15. June 1999 by JAG
...
// Modification C.Henquin, 1998-06-12: Add support for wildcard
	...
	if ( lpszCgiSuccessUrl!=NULL ) free(lpszFirstReceivedData);
...
void fileGetSubject(char *fn)
{
	...
	if ( infile!=NULL ) {
		...
		fgets(subject, SUBJECT_SIZE, infile);
		if ( strlen(subject) ) {
